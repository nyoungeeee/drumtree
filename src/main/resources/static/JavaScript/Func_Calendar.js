...
function updateReservation(type, member, idx) {
		...
		$.ajax({
	        ...
	        success: function(data) {
	        	if (data.rt=="UpdateRsv_FAIL001") {
	        		...
	        		if (loginGrade==99) {
	        			approvalReservation(idx, member);
	        		}
	        		else {
...
function deleteReservation(type, member, idx) {
	        ...
	        success: function(data) {
	    		...
	    		if (startCheckArray.length==0) {
	    			...
	    			for (var arrayIdx = 0; arrayIdx < startCheckArray.length; arrayIdx++) {
	        			...
	        			divHTML += "<a style='float:right;'>" + maskingText(memberNameCheckArray[arrayIdx]) + "</a>";
	        			divHTML += "<br>";
	        		}
	    		}
	    		divHTML += "</div>";
